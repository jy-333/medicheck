...
export default function CaregiverHome({
  ...
  onNavigate: (tab: "history" | "wellness" | "family") => void;
}) {
      ...
      <div className="px-5 pt-6">
        <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-6 text-center">
          ...
          <p className="text-amber-600 text-sm">
            Go to the Family tab to pair with an Elder.
          </p>
        </div>
      </div>
    );
  ...
  const recentMissed = elderLogs
    .filter(
      (l) => l.status === "missed" && new Date(l.date + "T00:00:00") >= weekAgo,
    )
    .sort(
  ...
  // low stock
  const lowStock = elderRx.filter(
    (p) => daysOfSupplyRemaining(p, elderLogs) <= 3,
  );
      ...
      {lowStock.length > 0 && (
        <div className="bg-amber-50 border-2 border-amber-200 rounded-3xl p-4 mb-4">
          ...
          {lowStock.map((p) => (
            <div
              key={p.id}
              className="flex items-center justify-between text-amber-700 text-sm py-1"
            >
              <span>{p.medicineName}</span>
              <span className="font-semibold">
                {daysOfSupplyRemaining(p, elderLogs)} days left
              </span>
            </div>
          ))}
        </div>
